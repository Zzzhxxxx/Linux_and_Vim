...
- [2.1.2 删除文本](#212-删除文本)~[2.1.4 粘贴文本](#214-粘贴文本)中介绍的操作我都称之为`<operator>`（当然强大的Vim不只有这几种`<operator>`），不难发现Vim中的很多操作都是`<operator> <number> <motion>`的自由组合。明白这点后，您已经对Vim有一定理解了！
...
## 2.4 命令模式
- 命令模式以`:`或`/`开始，以`<Enter>`结束。
- 为了和其他模式区分并让命令看起来简洁，我会在下表中列出`:`和`/`而不列出`<Enter>`，您在输入时请不要忘记`<Enter>`。
