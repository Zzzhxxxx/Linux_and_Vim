...
- 需要注意的是，如果之前是以 `dd`/`5yy` 这种以行为单位的操作实现删除/复制功能之后，`p` 会直接在当前行的下一行粘贴，`P` 会直接在当前行的上一行粘贴，很方便（之后提到的Line Visual Mode也会有此特性）。
...
| `g` | 全局，对文本行中所有匹配的字符串执行查找和替换操作。若省略g，则只替换每个文本行中第一个匹配的字符串。 |
