...
- 注：`chmod 755` 中的三个数字分别代表文件权限的设置，具体含义如下：
	- 第一个数字 7：文件所有者 User 的权限。7 代表所有者拥有 **读 r=4、写 w=2 和执行 x=1** 的权限，总和为 $7=4+2+1$。
	- 第二个数字 5：文件所在群组 Group 的权限。5 代表群组拥有 **读 r=4 和执行 x=1** 的权限，总和为 5=4+0+1。
	- 第三个数字 5：其他用户 Others 的权限。5 代表其他用户拥有 **读 r=4 和执行 x=1** 权限，总和为 5=4+0+1。
- 因此，`chmod 755` 意味着：文件所有者可以读、写和执行文件；群组可以读和执行文件，但不能写；其他用户也可以读和执行文件，但不能写。
