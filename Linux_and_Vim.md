...
- 注：`chmod 755` 的三个数字分别表示不同用户的文件权限，数字由 **读 (r=4)**、**写 (w=2)** 和 **执行 (x=1)** 的加和值组成：
	- 第一个数字 7：代表文件所有者 User 的权限，**7=4+2+1**，即读、写、执行权限。
	- 第二个数字 5：代表群组 Group 的权限，**5=4+1**，即读和执行权限。
	- 第三个数字 5：代表其他用户 Others 的权限，**5=4+1**，即读和执行权限。
