...
- 注：`chmod 755` 中的三个数字分别代表文件权限的设置，具体含义如下：
	...
	- 第二个数字（5）：文件所在组群体（Group）的权限。5 代表组群体拥有 读（r = 4） 和 执行（x = 1） 权限，总和为 5（4 + 0 + 1）。
	- 第三个数字（5）：其他用户（Others）的权限。5 代表其他用户拥有 读（r = 4） 和 执行（x = 1） 权限，总和为 5（4 + 0 + 1）。
- 因此，`chmod 755` 意味着：文件所有者可以读、写和执行文件；组群体可以读和执行文件，但不能写；其他用户也可以读和执行文件，但不能写。

![image](https://github.com/user-attachments/assets/c775e5c1-c0d7-4cfb-b317-d8498b64dbd9)

- 以这张图片为例（`ll` 是之前提到的 `ls -l` 命令的缩写），add.v 文件的权限是 664，hello 文件夹的权限是 775，`d` 代表 directory，`-` 代表普通文件。
